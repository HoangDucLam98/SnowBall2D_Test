...
public partial class Ferr2DT_PathTerrain : MonoBehaviour, Ferr2D_IPath, IBlendPaintable
{
    ...
    private void CreateVertColors()
    {
        ...
        {
            case Ferr2DT_ColorType.SolidColor:
                ColorVertsSolid(mesh, vertexColor);
                break;
            case Ferr2DT_ColorType.Gradient:
                ColorVertsGradient(mesh, vertexGradient, vertexGradientAngle);
                break;
            case Ferr2DT_ColorType.DistanceGradient:
                ColorVertsDistanceGradient(mesh, pathData.GetFinalPath(), pathData.Closed, vertexGradient, vertexGradientDistance);
                break;
            case Ferr2DT_ColorType.PreserveVertColor:
                ColorVertsRecolor(mesh, recolorTree);
                break;
        }
    }
    ...
    void Set2DColliderSettings<T>(List<T> aColliders) where T : Collider2D
    {
        ...
        {
            aColliders[i].isTrigger = true;
#if UNITY_5_5_OR_NEWER
            aColliders[i].usedByEffector = true;
#endif
        }
    }
    ...
    void AddConnectedColliderCap(bool aRightSide, bool aInner, bool aSharp, EdgeSegment aToSeg, ref List<Vector2> aPoints, List<Vector2> aNormals)
    {
        ...
        if (dot < -0.95f)
        { // near parallel facing the same direction
        }
        else if (dot > 0.75f)
        { // near parallel facing towards eachother
